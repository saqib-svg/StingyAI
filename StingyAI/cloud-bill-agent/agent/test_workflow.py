...
def test_scenarios_a_to_d():
    ...
    script_c = [
        json.dumps({
            ...
            "reason": "Inspect latest 10:30 traffic"
        }),
        json.dumps({
            "type": "final",
            "response": "Latest traffic observation (10:30 RPM=5200) is higher than 08:00 (RPM=900). Cost reduction is UNSAFE."
        })
    ]
    ...
    script_d = [
        ...
        json.dumps({
            "type": "final",
            "response": "Acknowledged previous scale action act-784 failed with capacity_unavailable. Evaluated current state and successfully scaled payment-api to 4 instances."
        })
    ]
    res_d = CloudAgent(hf_client=MockHFWorkflowClient(script_d), tool_client=tool_client).run("Scenario D")
    assert len(res_d["actions"]) == 1
    assert res_d["actions"][0]["arguments"]["instances"] == 4

    # Reset simulator back to Scenario A after tests complete
    simulator.load_scenario("a")
