...
def test_agent_adaptation_and_no_duplicate_retry():
    print("\n[4b] Testing Rejection Adaptation & Duplicate Retry Protection ...")
    simulator.load_scenario("a")
    tool_client = get_test_api_client()

    # Script simulates model proposing stop_service, getting rejected, trying stop_service again (blocked), then scaling to 1
    script = [
        json.dumps({
            "type": "tool_call",
            "tool": "stop_service",
            "arguments": {"service_id": "reports-worker"},
            "reason": "Attempting unsafe stop"
        }),
        # Model tries identical stop_service again
        json.dumps({
            "type": "tool_call",
            "tool": "stop_service",
            "arguments": {"service_id": "reports-worker"},
            "reason": "Attempting duplicate stop"
        }),
        # Model adapts to valid scale_service
        json.dumps({
            "type": "tool_call",
            "tool": "scale_service",
            "arguments": {"service_id": "reports-worker", "instances": 1},
            "reason": "Adapting to scale down to min_instances=1"
        }),
        json.dumps({
            "type": "final",
            "response": "Adapted strategy successfully and scaled reports-worker to 1 instance."
        })
    ]

    mock_hf = MockHFClient(script)
    agent = CloudAgent(hf_client=mock_hf, tool_client=tool_client)
    res = agent.run("Optimize reports-worker.")

    assert res["success"] is True
    assert res["status"] == "completed"
    # Duplicate retry check should have intercepted line 2
    validation_traces = [e for e in res["execution_trace"] if e.get("phase") == "validation" and "Prevented retrying" in e.get("info", "")]
    assert len(validation_traces) == 1
    assert res["verification"][0]["verification_success"] is True
    assert res["verification"][0]["observed_state"]["instances"] == 1
    print("[OK] Rejection adaptation & duplicate retry protection verified successfully.")
...
def test_scenario_b():
    ...
    script = [
        ...
        json.dumps({
            ...
            "reason": "High traffic (4200 RPM, latency 260ms near 300ms limit); scaling orders-api from 4 to max (8) instances"
        }),
        json.dumps({
            "type": "final",
...
def test_scenario_c():
    ...
    script = [
        json.dumps({
            ...
            "reason": "Fetch latest 10:30 traffic metrics for checkout service"
        }),
        json.dumps({
            "type": "final",
            "response": "Latest traffic observation (10:30) shows 5200 RPM (up from earlier 08:00 observation of 900 RPM). Reducing cost by scaling down is UNSAFE under current high traffic load."
        })
    ]
...
def test_scenario_d():
    ...
    script = [
        json.dumps({
            ...
            "reason": "Check current state of payment-api service after reported previous action failure (act-784)"
        }),
        json.dumps({
            "type": "tool_call",
            "tool": "scale_service",
            "arguments": {"service_id": "payment-api", "instances": 4},
            "reason": "Previous scale_up to 5 failed (capacity_unavailable). Payment service is overloaded at 91% CPU; scaling to 4 instances."
        }),
        json.dumps({
            "type": "final",
            "response": "Acknowledged previous scale action act-784 to 5 instances failed with capacity_unavailable. Evaluated current overloaded state (CPU 91%, latency 410ms) and successfully scaled payment-api to 4 instances."
        })
    ]
    ...
    assert res["success"] is True
    assert len(res["actions"]) == 1
    assert res["actions"][0]["arguments"]["instances"] == 4
    assert res["verification"][0]["verification_success"] is True
    assert res["verification"][0]["observed_state"]["instances"] == 4
    print("[OK] Scenario D verified: Agent acknowledged previous failed action and safely scaled to available capacity (4).")
...
def main():
    ...
    test_agent_adaptation_and_no_duplicate_retry()
